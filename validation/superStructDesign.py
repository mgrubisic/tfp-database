...
#               Specifications for bearings: Tm, mu1, R1, zeta
...
#               Constraints are only placed on physical design (positive mu and R)
...
def design():
    ...
    # SaTm is collected according to the design spectrum
    Ts  = param['S1']/Ss
    # param['Tm'] = param['TmRatio']*Ts
    SaTm    = param['S1']/param['Tm']

    # # collect SaTm from the ground motion in the DoE run to ensure point added is exact
    # gmDir                     = '../groundMotions/PEERNGARecords_Unscaled/'
    # resultsCSV                = 'combinedSearch.csv'
    # SaTm  = gmSelector.getST(gmDir, resultsCSV, gmFilename, gmDefScale, 
    #   param['Tm'], 32, 133, 290, 111)
    ...
    # all SaTm for DoE run is GMSaTm
    Dm      = g*(SaTm/Bm)*(param['Tm']**2)*param['gapRatio']/(4*pi**2)

    x       = g*(SaTm/Bm)*(param['Tm']**2)/(4*pi**2)
    # x         = Dm
